/**
 * 경북대 학과 정보 → departments 테이블 seed (멱등)
 *
 * 기존 데이터를 모두 지우고 다시 넣는다.
...
const DEPARTMENTS = [
  ...
  { college: '공과대학', departments: ['토목공학과', '기계공학과', '화학공학과', '금속신소재공학과', '건축학부', '전기공학과', '환경공학과', '응용화학공학과', '건설환경에너지공학부'] },
  ...
  for (const { college, departments } of DEPARTMENTS) {
    for (const name of departments) {
      try {
        ...
        console.error(`실패 (${college}/${name}):`, err.message);
      }
    }
  }
